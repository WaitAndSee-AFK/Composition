...
class GameViewModel(
    ...
    private val generateQuestionsUseCase = GenerateQuestionUseCase(repository)
    ...
    // Лайфдата с количеством правильных ответов по всем вопросом
    ...
    private fun generateQuestion() {
        _question.value = generateQuestionsUseCase(gameSettings.maxSumResult)
    }

    private fun formatTime(millisUntilFinished: Long): String {
        val seconds = millisUntilFinished / MILLIS_IN_SECONDS
        val minutes = seconds / SECONDS_IN_MINUTE
        val leftSecond = seconds - (minutes * SECONDS_IN_MINUTE)
        return String.format(format = "%02d:%02d", minutes, leftSecond)
    }
